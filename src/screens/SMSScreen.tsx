...
const SMSScreen: React.FC = () => {
  ...
  const groupPaymentsByDate = useCallback((paymentList: Payment[]): GroupedPayments[] => {
    ...
    return Object.entries(groups)
      .map(([dateKey, groupPayments]) => ({
        ...
        payments: groupPayments.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()),
      }))
  ...
  const loadSMSPayments = useCallback(async () => {
    try {
      ...
      const smsPayments = allPayments.filter(payment => 
        payment.type === 'sms' || payment.isFromSMS
      );
  ...
  const renderPaymentItem = (payment: Payment) => {
    return (
      <View key={payment.id} style={[styles.paymentItem, { backgroundColor: theme.colors.surface }]}>
    ...
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
        <View style={styles.header}>
          <Text style={[styles.title, { color: theme.colors.text }]}>SMS Payments</Text>
      ...
      <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
        ...
        <View style={styles.centerContainer}>
          ...
          <Text style={[styles.emptySubtitle, { color: theme.colors.textSecondary }]}>
            Enable SMS auto-parsing in Settings to automatically detect payments from SMS messages
          </Text>
        </View>
      </SafeAreaView>
    );
